...
def read_one(db: Session, customer_id):
    try:
        item = db.query(model.Customer).filter(model.Customer.customer_id == customer_id).first()
        if not item:
...
def update(db: Session, customer_id, request):
    try:
        item = db.query(model.Customer).filter(model.Customer.customer_id == customer_id)
        if not item.first():
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Id not found!")
